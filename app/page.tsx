        ...
        {data?.map((user) => (
          <>
            <Card
              title={user.name}
              subTitle={user.url}
              className="max-w-5rem col-span-4 md:col-span-2 sm:col-span-1"
              key={user.id}
            >
              <p className="m-0">{user.descriptions}</p>
            </Card>
          </>
        ))}
      </div>
    ...
    <React.Fragment>
      <main className="min-h-screen min-w-screen flex-col items-center justify-between p-8 grid grid-cols-4 gap-4 sm:gap-1 md:gap-2">
        {Array.from({ length: 12 }, (_, i) => i + 1).map((id) => (
          <div
            className="w-[200px] h-[200px] col-span-4 md:col-span-2 lg:col-span-1 bg-slate-500"
            key={id}
          >
            <Skeleton className="h-6 w-1/2 m-1"></Skeleton>
            <Skeleton className="h-6 w-1/4 m-2"></Skeleton>
            <Skeleton className="w-2/3 h-2/3 rounded-md m-2"></Skeleton>
          </div>
        ))}
      </main>
    </React.Fragment>
